...
import { DocumentType } from "../../interfaces/types";
...
export class CreateClientDto {
    ...
    public static create(object: { [key: string]: any }): [ string?, CreateClientDto? ] {

        const { documentType, documentNumber, firstName, firstSurname, birthDate, addresses, secondName, secondSurname } = object;

        if (!documentType) return ['Property documentType is required'];
        if (!['CC', 'CE', 'TI', 'NIT'].includes(documentType)) return ['Invalid documentType'];
        if (!documentNumber) return ['Property documentNumber is required'];
        if (!firstName) return ['Property firstName is required'];
        if (!firstSurname) return ['Property firstSurname is required'];

        // Validate birthDate
        if (!birthDate) return ['Property birthDate is required'];
        const parsedDate = new Date(birthDate);
        if (isNaN(parsedDate.getTime())) return ['Invalid birthDate']; // Checks if parsedDate is an Invalid Date

        // Validate addresses
        if (!addresses || !Array.isArray(addresses) || addresses.length === 0) return ['Property addresses is required and must be a non-empty array'];
        ...
        return [undefined, new CreateClientDto(documentType, documentNumber, firstName, firstSurname, birthDate, addresses, secondName, secondSurname)];
    }
}